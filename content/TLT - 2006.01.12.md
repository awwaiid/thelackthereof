...
This is a bad thing. Or at least annoying. At the end of each state of the application it must explicitly decide what its next state will be. It's like having to having to program with GOTO instead of function calls.

Continuations change all that. While I won't go into the details here, using continuations switches things around from the programmer's point of view. It's like you suddenly have a new function, display_html_and_wait_for_input(). Your program never exits, just like a desktop application. I didn't come up with this idea, [http://www.google.com/search?q=continuation+based+web+programming google around a bit] for further details.
