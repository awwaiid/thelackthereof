...
Under all of that, from what I've seen on the internets these LLMs are not actually all that good at actually playing tic-tac-toe or other 2D spatial games (like Chess) in the first place. But that isn't really the problem so far -- the problem right now is that the VLM *wants* to move in the center but can't figure out where the center is.
