...
function tweakMarkdown(file) {
  ...
  content = content.replace(/audio:(\S+)/gm, '<audio controls src="/$1.mp3" />');
  // content = content.replace(/audio:(\S+)/gm, '<av-waveform audio-src="/$1.mp3" />');
  ...
  content = content.replace(
    /\[\[([^\]]*)\]\]?/gm,
    (match, p1) => {
      const url = p1.replace(/ /g, '_').toLowerCase();
      return `[${p1}](/${url})`;
    }
  );
